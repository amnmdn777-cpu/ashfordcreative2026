...
import type { LeadTimelineCall } from "@rep/lib/api";
import { fmtDateTime } from "@rep/lib/api";

/**
 * Timeline rendering for a single call (or voicemail). Composes:
 *   - Direction + status icon (✅ connected, ❌ missed, 🔊 voicemail)
 *   - Audio playback when a presigned URL is available
 *   - Auto-generated transcript (collapsed by default — could be long)
 *   - GPT summary + bulleted talking points / next actions
 *
 * Designed to plug into the existing LeadDetail timeline list — the
 * outer `<ol>` provides the list semantics, this component renders the
 * `<li>` content.
 */
...
export function CallTimelineList({ calls }: { calls: LeadTimelineCall[] }) {
  ...
  return (
    <div className="mb-4">
      <h3 className="font-serif text-sm uppercase tracking-wide text-muted-foreground mb-2">
        Calls
      </h3>
